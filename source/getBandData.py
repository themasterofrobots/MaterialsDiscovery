...
FILENAME = 'knownQD_list'

all_filters = ['{"elements":{"$in":["N", "P", "As", "Sb", "Bi"], "$all":["Ga"]}, "nelements":{"$lte":2}, "band_gap": {"$gt": '+ str(BAND_GAP_LB) + ', "$lt": ' + str(BAND_GAP_UB) +'}}',
...
#all_filters = ['{"elements":{"$nin":["Cd", "Hg", "Cr", "Pb"]}, "nelements":{"$lte":3}, "band_gap": {"$gt": '+ str(BAND_GAP_LB) + ', "$lt": ' + str(BAND_GAP_UB) +'}}']
...
def get_material(query_info):
	try:
		...
		with MPRester() as mp:
			...
			if (bandstruct.is_metal() or not(bandinfo["direct"])):
				return
...
for i_filter in range(len(all_filters)):
	print 'Started using filter ' + str(i_filter+1) + '.'
	get_materials(all_filters[i_filter])
